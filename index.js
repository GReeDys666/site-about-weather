...
showWeatherData = (weatherData) => {
    ...
    document.querySelector(".current-stats__temp-max").innerText = Math.round(weatherData.main.temp_max) + " °C";
    document.querySelector(".current-stats__temp-min").innerText = Math.round(weatherData.main.temp_min) + " °C";
    document.querySelector(".current-stats__wind-speed").innerText = Math.round(weatherData.wind.speed) + "mph";
...
showHoursData = (data) => {
    ...
    for (let i = 0; i < data.list.length; i++) {
        document.querySelectorAll(".weather-by-hour__item img")[i].src = `${iconBaseUrl}${data.list[i].weather[0].icon}${iconFormat}`;
        document.querySelectorAll(".degree")[i].innerText = Math.round(data.list[i].main.temp) + " °C";
...
showDefaultData = (data) => {
    ...
    for (let i = 0; i < data.list.length; i++) {
        ...
        document.getElementsByClassName("next-5-days__date")[i].innerText = day;
        document.querySelectorAll(".low-temperature")[i].innerText = Math.round(data.list[i].temp.min) + "°";
        document.querySelectorAll(".high-temperature")[i].innerText = Math.round(data.list[i].temp.max) + "°";
        document.querySelectorAll(".humidity")[i].innerText = data.list[i].humidity + "%";
        document.querySelectorAll(".next-5-days__wind .wind-speed")[i].innerText = (data.list[i].speed).toFixed(1) + "mph";
        document.querySelectorAll(".next-5-days__icon .days-icon")[i].src = `${iconBaseUrl}${data.list[i].weather[0].icon}${iconFormat}`;
